...
define('Facets.Views', ['Cart', 'Facets.Helper', 'Categories'], function (Cart, Helper, Categories)
{
	...
	var filePathLocation = '/SCA QuickStart - Basic/ShopFlow/js/src/app/modules/Facets/Facets.Views.js';
	...
	,	initialize: function (options)
		{
			...
			//console.log('this.category: ' + '\n' + JSON.stringify(this.category, 'key', '\t'))
			//console.log(filePathLocation + '\n' + 'initialize >> this.translator: ' + '\n' + JSON.stringify(this.translator, 'key', '\t'))

			var isOrderPage = "Item Types" == this.category.itemid;
	...
	, 	getClientId: function(fragment)
		{
			...
			for (var i = fragmentArray.length - 1; i >= 0; i--) {
				if(fragmentArray[i].match('client')) {
					return fragmentArray[i].split('=')[1].split("&")[0];
				}
			};
		}
	,	getPath: function ()
	...
	,	renderFacets: function (url)
		{
			var self = this
			var	translator = Helper.parseUrl(url, this.options.translatorConfig);
			//translator.facets[0]['priority'] = 10
			//translator.facets[0]['behavior'] = 'single'

			var	facets = this.model.get('facets');

			//console.log(filePathLocation + '\n' + 'renderFacets >> translator:' + '\n' + JSON.stringify(translator, 'key', '\t'))
			...
			if (category_string)
			{
				...
				_.each(Categories.getBranchLineFromPath(category_string), function (cat)
				{
					if (Backbone.history.fragment.split("?").length > 1){
						category_path += '/'+cat.urlcomponent + self.client;
					} else {
